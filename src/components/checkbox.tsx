...
const colors = {
  ...
  yellow:
    "[--checkbox-check:var(--color-white)] [--checkbox-checked-bg:var(--color-yellow-500)] [--checkbox-checked-border:var(--color-yellow-600)]/90",
  fuchsia:
...
export const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  ({ color = "dark/zinc", className, label, description, ...props }, ref) => (
    <CheckboxField>
      <Headless.Checkbox
        ref={ref}
        data-slot="control"
        ...
        className={clsx(
          className,
          "group inline-flex focus:outline-hidden cursor-pointer"
        )}
      >
      ...
      {label && (
        <Headless.Label
          data-slot="label"
          className="text-sm leading-6 text-zinc-950 dark:text-white"
        >
          {label}
        </Headless.Label>
      )}
